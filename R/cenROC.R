...
#'
#' @description  This function computes the time-dependent ROC curve (receiver operating characteristic curve) for right censored survival data using the cumulative sensitivity and dynamic specificity definitions.
...
#' @return    \code{ROC      } The vector of the estimated ROC at U for the given \code{t}. These will be numeric numbers between
#' @return    \code{         }  \code{       0} and \code{1}.
...
cenROC <- function(Y, M, censor, t, U = NULL, h = NULL, bw = "NR",  method = "tra", ktype = "normal", ktype1 = "normal", B = 0, alpha = 0.05, plot = TRUE)
  {
    ...
    if (!is.vector(Y, mode = "numeric") |
      ...
      stop("All the numeric vectors Y, M and censor should be specified")
    else{
    ...
    if (B > 0){
      ...
      for (i in 1:B){
        ...
        estim1 <- RocFun(U = U, D = Dt, M = dat$M, method = method, bw = bw, ktype = ktype)
        aucb[i] <- 1 - estim1$auc
        rocb[, i] <- estim1$roc
      }
    ...
    if (plot == "TRUE") {
      plot(c(0, U, 1), c(0, (estim$roc), 1), type = "l", lwd = 2, col.lab = "blue", col = "blue",
           xlab = "False positive rate", ylab = "True positive rate")
      abline(coef = c(0,1))
    }

    return(list(AUC = AUC, ROC = ROC, U = U, bw = estim$bw))
  }
